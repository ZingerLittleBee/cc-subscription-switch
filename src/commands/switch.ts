...
export async function switchCommand(
  ...
  if (config.accounts.length === 0) {
    ...
    return switchCommand(claudeArgs, forceSelect);
  }

  let selectedName: string;

  // If single account and not forcing select, use it directly
  if (config.accounts.length === 1 && !forceSelect) {
    selectedName = config.accounts[0].name;
  } else {
